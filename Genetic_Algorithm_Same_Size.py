...
def Leg_Length_Mir (mir_gene, limit = 6):
    tmp_group = []
    tmp = []
...
def ScoreFunction ( pop , path , mir_scores, inhibitors ):  # Takes in the population, pathway, and the mir_scores dictionary as inputs. Uses the Score function to build the fitness list. 
    curr_fitness = []
    for i in range(len(pop)):
        curr_fitness.append(Score_Covering(pop[i], inhibitors))  #Need to Change to selectable type, for Score - need to add (path, mir_scores) as paramaters
    return curr_fitness
...
o_file = open("/Users/nithintumma/Desktop/Output/output_ISEF.txt", "r+")   # The output file
fit_file = open("/Users/nithintumma/Desktop/Output/fitness_ISEF.csv", "r+") # The fitness output file
maxfit_file = open("/Users/nithintumma/Desktop/Output/maxfit_ISEF.csv", "r+") #the Maximum fitness output file 
...
for i in range(300):
	population.append(getMirs(max_gene_len + 1))
